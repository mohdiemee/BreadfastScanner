                ...
                if (bitmap != null) {
                    ...
                    for (deal in deals) {
                        if (!deal.isAssigned) {
                            ...
                            val words = cleanName.split(Regex("\\s+")).map { it.trim() }.filter { it.length >= 4 }.distinct().take(4)
                            val matchCount = words.count { word -> screenText.contains(word, ignoreCase = true) }
